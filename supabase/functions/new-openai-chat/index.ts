...
async function waitForRun(threadId: string, runId: string) {
  let delay = 200;
  while (true) {
    const run = await fetchWithRetry(
...
async function getLatestAssistantReply(threadId: string): Promise<string> {
...
/*──────────────────────────  Action handlers  ───────────────────────────*/
async function createThread(): Promise<Response> {
  const res = await fetchWithRetry(`${OPENAI_BASE}/threads`, {
    method: 'POST',
    headers: { ...ASSISTANTS_BETA, Authorization: `Bearer ${openAIApiKey}` }
  }).then(r => r.json());

  return json({ threadId: res.id });
}

async function sendImageAndRun(
  {
    threadId,
    assistantId,
    fileData,
  ...
  // 3 — run
  const { id: runId } = await fetchWithRetry(
...
async function generateImages(
  {
    ...
    baseFileData: string;
    prompt: string;
    options: { number?: number; size?: string; quality?: string; output_format?: string };
  }
) {
  if (!prompt || prompt.length > 4000) throw new Error('Invalid prompt');
  const safePrompt = prompt.replace(/[<>]/g, '');

  const tasks = Array.from({ length: options.number ?? 1 }, async () => {
    const form = new FormData();
    form.append('model', 'gpt-image-1');
    form.append('image', b64ToBlob(baseFileData));
  ...
  return json({ images: await Promise.all(tasks) });
}
