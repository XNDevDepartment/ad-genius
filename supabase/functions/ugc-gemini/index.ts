...
// ---------- LOG ----------
const log = (step: string, meta?: any) => console.log(`[UGC-GEMINI] ${step}${meta ? ` - ${JSON.stringify(meta)}` : ""}`);
...
async function generateImages(jobId: string, supabase: SupabaseClient) {
  ...
  const { data: job, error: claimErr } = await supabase
    ...
    .eq("id", jobId)
    .eq("status", "queued")
    .eq("model_type", "gemini") // Ensure we're processing a Gemini job
    .select()
    .single();
...
async function generateSingleImageWithGemini(job: any, index: number, sourceImageUrl: string | null, supabase: SupabaseClient) {
...
async function getJob(userId: string, jobId: string, supaUser: SupabaseClient) {
  const { data: job, error } = await supaUser
    ...
    .eq("id", jobId)
    .eq("user_id", userId)
    .eq("model_type", "gemini") // Only return Gemini jobs
    .single();
...
async function cancelJob(userId: string, jobId: string, supabase: SupabaseClient) {
  const { data: job, error } = await supabase
    ...
    .eq("id", jobId)
    .eq("user_id", userId)
    .eq("model_type", "gemini") // Only cancel Gemini jobs
...
async function getActiveJob(userId: string, supabase: SupabaseClient) {
  const { data: job, error } = await supabase
    ...
    .select("*")
    .eq("user_id", userId)
    .eq("model_type", "gemini") // Only get Gemini jobs
    .in("status", ["queued", "processing"])
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
...
async function recoverQueued(supabase: SupabaseClient) {
  ...
  const { data: jobs, error } = await supabase
    ...
    .select("id,status")
    .eq("status", "queued")
    .eq("model_type", "gemini") // Only recover Gemini jobs
